...
export const RecursosMovilizados: React.FC = () => {
  ...
  const [cantones, setCantones] = useState<Array<{ id: number; nombre: string; provincia_id: number }>>([]);
  const [parroquias, setParroquias] = useState<Array<{ id: number; nombre: string; canton_id: number }>>([]);
  const [instituciones, setInstituciones] = useState<Array<{ id: number; nombre: string; siglas: string }>>([]);
  const [recursoGruposCat, setRecursoGruposCat] = useState<Array<{ id: number; nombre: string }>>([]);
  const [recursoGruposCatStatus, setRecursoGruposCatStatus] = useState<'idle' | 'loading' | 'succeeded' | 'failed'>('idle');
  const [recursoTiposCat, setRecursoTiposCat] = useState<Array<{ id: number; nombre: string; recurso_grupo_id: number }>>([]);
  const [recursoTiposCatStatus, setRecursoTiposCatStatus] = useState<'idle' | 'loading' | 'succeeded' | 'failed'>('idle');
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await authFetch(`${apiBase}/recurso_grupos/categoria/2`, { headers: { accept: 'application/json' } });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await authFetch(`${apiBase}/recurso-tipos/categoria/2`, { headers: { accept: 'application/json' } });
        const data = res.ok ? await res.json() : [];
        setRecursoTiposCat(Array.isArray(data) ? (data as Array<{ id: number; nombre: string; recurso_grupo_id: number }>) : []);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await authFetch(`${apiBase}/instituciones`, { headers: { accept: 'application/json' } });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const p = await authFetch(`${apiBase}/provincias/emergencia/${selectedEmergenciaId}`, { headers: { accept: 'application/json' } });
      ...
      try {
        if (datosLogin?.provincia_id) {
          const c = await authFetch(`${apiBase}/provincia/${datosLogin.provincia_id}/cantones/`, { headers: { accept: 'application/json' } });
  ...
  const fetchRows = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setRows(Array.isArray(data) ? data as RecursoMovilizadoListItem[] : []);
      }
    } catch {
      setRows([]);
    } finally {
  ...
  const handleSave = async (payload: Partial<RecursoMovilizadoPost> & { id?: number; recurso_id?: number }) => {
    const creator = datosLogin?.usuario_login || datosLogin?.usuario_descripcion || 'usuario';
    const selectedTipo = recursoTiposCat.find(t => t.id === Number(payload.recurso_tipo_id));
    const resolvedGrupoId = selectedTipo?.recurso_grupo_id ?? Number(payload.recurso_grupo_id ?? 0);
    const body: RecursoMovilizadoPost = {
      ...
      fecha_inicio: payload.fecha_inicio ?? new Date().toISOString().substring(0, 19),
    ...
    const isEdit = !!id;
    const url = isEdit ? `${apiBase}/recursos_movilizados/${id}` : `${apiBase}/recursos_movilizados`;
  ...
  const renderForm = (item: Partial<RecursoMovilizadoPost>, onChange: (e: any) => void) => {
    const onDateChange = (date: string | Date | Date[] | null, field: keyof RecursoMovilizadoPost) => {
      if (!date) return;
      ...
      if (Array.isArray(date)) {
        ...
        if (!isNaN(parsed.getTime())) iso = parsed.toISOString().substring(0, 19);
      } else {
    ...
    const onDropdownChange = (e: { value: any }, field: keyof RecursoMovilizadoPost) => {
      onChange({ target: { name: field, value: e.value } });
    };

    const onNumberChange = (e: { value: number | null }, field: keyof RecursoMovilizadoPost) => {
      onChange({ target: { name: field, value: e.value || 0 } });
    };

    const grupoOptions = (recursoGruposCat || []).map(g => ({ label: g.nombre, value: g.id }));
    const tipoOptions = (recursoTiposCat || []).map(t => ({ label: t.nombre, value: t.id }));
    ...
    const handleProvinciaChange = async (provId: number | null) => {
      ...
      setParroquias([]);
      if (!provId) { setCantones([]); return; }
      try {
        const res = await authFetch(`${apiBase}/provincia/${provId}/cantones/`, { headers: { accept: 'application/json' } });
    ...
    const handleCantonChange = async (canId: number | null) => {
      ...
      onChange({ target: { name: 'parroquia_id', value: 0 } });
      if (!canId) { setParroquias([]); return; }
      try {
        const res = await authFetch(`${apiBase}/parroquias/canton/${canId}`, { headers: { accept: 'application/json' } });
       ...
       <div className="row">
        ...
        <div className="field col-6 md:col-4">
          ...
          <div className="d-flex align-items-center" style={{ gap: 8 }}>
            <InputSwitch checked={!!item.disponible} onChange={(e) => onChange({ target: { name: 'disponible', value: e.value } })} />
          </div>
        </div>
        </div>
        <div className="field col-12 md:col-4">
          ...
          <Dropdown
            value={typeof item.institucion_id === 'number' ? item.institucion_id : null}
        ...
        <div className="field col-12 md:col-4" style={{ display: 'none' }}>
          ...
          <Dropdown
            ...
            placeholder={recursoGruposCatStatus === 'loading' ? 'Cargando...' : 'Seleccionar grupo'}
        ...
        <div className="field col-12 md:col-4">
          ...
          <Dropdown
            ...
            placeholder={recursoTiposCatStatus === 'loading' ? 'Cargando...' : 'Seleccionar tipo'}
        ...
        <div className="field col-12 md:col-4" style={{ display: 'none' }}>
          ...
          <InputNumber value={2} onValueChange={(e) => onNumberChange(e, 'recurso_categoria_id')} className="w-full" />
        </div>
        <div className="row">
          <div className="field col-6 md:col-6">
          <label>Fecha Inicio *</label>
          <Calendar value={item.fecha_inicio ? new Date(item.fecha_inicio) : null} onChange={(e) => onDateChange(e.value as Date, 'fecha_inicio')} showIcon dateFormat="dd/mm/yy" className="w-full" />
        </div>
        <div className="field col-6 md:col-6">
          <label>Fecha Fin</label>
          <Calendar value={item.fecha_fin ? new Date(item.fecha_fin) : null} onChange={(e) => onDateChange(e.value as Date, 'fecha_fin')} showIcon dateFormat="dd/mm/yy" className="w-full" />
        </div>
        </div>
  ...
  const resolveItemForEdit = async (row: RecursoMovilizadoListItem): Promise<Partial<RecursoMovilizadoPost> & { id?: number; recurso_id?: number }> => {
    ...
    if (row.institucion) {
      const inst = instituciones.find(i => i.nombre === row.institucion || `${i.siglas ? i.siglas + ' - ' : ''}${i.nombre}` === row.institucion);
